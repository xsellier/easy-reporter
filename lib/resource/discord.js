...
class DiscordResource {
  ...
  constructor (rawOptions) {
    ...
    this.client.on('messageCreate', this._message_event_handler.bind(this, 'messageCreate'))
    this.client.on('messageReactionAdd', this._message_event_handler.bind(this, 'messageReactionAdd'))
    this.client.on('messageReactionRemove', this._message_event_handler.bind(this, 'messageReactionRemove'))
    this.client.on('guildMemberRemove', this._guild_event_handler.bind(this, 'guildMemberRemove'))
    this.client.on('guildMemberAdd', this._guild_event_handler.bind(this, 'guildMemberAdd'))
  ...
  _guild_event_handler(event_type, member) {
    if (member.user.id == this.options.client_id) {
      // Do not forward an event we created
      return
    }

    if (this.listeners[event_type] != null) {
      this.listeners[event_type].forEach((handler) => handler(member))
    }
  }

  _message_event_handler(event_type, message) {
