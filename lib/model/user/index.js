...
class User {
  ...
  static setUserBan(userId, banned) {
    return Promise.resolve()
      .then(() => resources.database.query(queries['set-user-ban'], [banned, userId]))
  }

  static favoriteProject(projectId, userId) {
    return Promise.resolve()
      .then(() => resources.database.query(queries['favorite-project'], [projectId, userId]))
  }
  ...
  static list (page) {
    return Promise.resolve()
      .then(() => resources.database.query(queries['list-user'], [resultPerPage, page * resultPerPage]))
  }

  static count () {
    return Promise.resolve()
      .then(() => resources.database.query(queries['count-user'], []))
  }
