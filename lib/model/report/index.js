...
class Report {
  static create (name, filename, debug, platform, version, title, cracked) {
    return Promise.resolve()
      .then(() => {
        var manual_report = /^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89ab][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$/.test(title)

        return resources.database.query(queries['create-report'], [name, filename, debug, platform, version, title, manual_report, cracked])
      })
  }
