...
class Report {
  static create (name, filename, debug, platform, version, title) {
    return Promise.resolve()
      .then(() => {
        ...
        return resources.database.query(queries['create-report'], [name, filename, debug, platform, version, title, manual_report])
      })
  }
  ...
  static list (name, debug, uploaded, deleted, fixed, manual, page) {
  ...
  static listAll (name, debug, uploaded, deleted, fixed, manual) {
    let customized_query = queries['list-all-report']
      .replace('__DELETED__', (deleted) ? ' NOT' : '')
      .replace('__FIXED__', (fixed) ? ' NOT' : '')
  ...
  static count (name, debug, uploaded, deleted, fixed, manual) {
