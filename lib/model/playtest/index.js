...
class Playtest {
  ...
  static updateDiscordRoleId(projectId, discordRoleId) {
    return Promise.resolve()
      .then(() => {
        return resources.database.query(queries['update-playtest-discord-role-id'], [discordRoleId, projectId])
      })
  }
  ...
  static getAvailableDiscordUser(projectId, amount) {
    return Promise.resolve()
      .then(() => resources.database.query(queries['get-available-discord-user'], [projectId, amount]))
  }

  static getAvailableSteamKey(projectId, amount) {
    return Promise.resolve()
      .then(() => resources.database.query(queries['get-available-steam-key'], [projectId, amount]))
  }

  static getFirstAvailableSteamKey(projectId) {
    return Promise.resolve()
      .then(() => Playtest.getAvailableSteamKey(projectId, 1))
      .then((result) => {
        return result.length > 0 ? result[0] : null
      })
  }

  static updateDiscordUserIdFeedback(projectId, discordUserId, feedback) {
    return Promise.resolve()
      .then(() => resources.database.query(queries['update-discord-user-id-feedback'], [feedback, projectId, discordUserId]))
  }

  static listDiscordUserWithoutFeedback(projectId) {
    return Promise.resolve()
      .then(() => resources.database.query(queries['list-discord-user-without-feedback'], [projectId]))
  }
}
