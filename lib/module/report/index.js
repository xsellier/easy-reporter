...
class Report {
  upload (token, payload) {
    ...
    return Promise.resolve()
      ...
      .then((data) => {
        return VersiontModel.isCracked(data.system.name)
          .then((isCracked) => {
            ...
            const emailData = {
              from: `Mailgun Sandbox <postmaster@${emailConfig.domain}>`,
              to: emailConfig.to,
              subject: `Bug report: ${data.application.version}`,
              text: `OS: ${JSON.stringify(data.system, null, 2)}\ndata: ${JSON.stringify(data.data, null, 2)}`
            }
