...
class Report {
  upload (token, applicationId, payload) {
    let filename = `/reports/${uuidv4()}.gz`

    return Promise.resolve()
      .then(() => ProjectModel.getSecretBySteamId(applicationId))
      .then((secretKey) => {
      ...
      .then((data) => {
        ...
        return VersionModel.isCracked(data.application.name, data.application.version)
          .then((isVersionCracked) => {
            if (isVersionCracked || data.data == null) {
              return true
            }

            return BugModel.isIgnored(data.application.name, data.data.error)
          })
          .then((isBugIgnored) => {
            if (isBugIgnored) {
              return true
...
Executable: ${data.system.executable}`
            ...
            return ProjectModel.getProjectEmailBySteamId(applicationId)
              .then((toEmail) => {
                resources.email.send(toEmail, subject, text)
              })
          })
          .then(() => {
            return EmailModel.create(data.application.name, (data.data != null ? data.data.error : 'Custom report'), data.application.version)
          })
          .catch((err) => {
            console.error(err)
          })
          .then(() => {
            return VersionModel.createOrUpdate(data.application.name, data.application.version, false)
          })
          .catch((err) => {
            console.error(err)
          })
          ...
          .then(() => {
            return BugModel.createOrForget(data.application.name, data.data.error, data.application.version, false)
          })
          .catch((err) => {
            console.error(err)
          })
  ...
  download (filename) {
    ...
    .then((data) => {
      return ReportModel.read(filename, true)
        ...
        .then((result) => { 
          return { fixed: result[0].fixed, version: result[0].version, title: result[0].title, filename, data }
        })
    })
  }
  ...
  listPlatform () {
    return ReportModel.listPlatform()
