...
class Report {
  upload (token, payload) {
    ...
    return Promise.resolve()
      ...
      .then((data) => {
        return VersionModel.isCracked(data.application.version)
          .then((isCracked) => {
            if (isCracked || data.data == null) {
              return true
            }

            return EmailModel.isSent(data.data.error, data.application.version)
          })
...
Executable: ${data.system.executable}`
          ...
          .then(() => EmailModel.create(data.data.error, data.application.version))
          .catch((err) => {
            console.error(err)
          })
          .then(() => {
            ...
            return ReportModel.create(filename, data.application.debug, data.system.name, data.application.version, data.data.error)
          })
          .catch((err) => {
            console.error(err)
          })
  ...
  list (debug, uploaded, deleted, fixed, page) {
    return Promise.all([
      ReportModel.list(debug, uploaded, deleted, fixed, page - 1),
      ReportModel.count(debug, uploaded, deleted, fixed)
    ])
    .then((result) => {
      return {
        list: result[0],
