...
class Report {
  async upload (token, applicationId, payload) {
    console.info(`Receiving new report ${applicationId}`)

    let filename = `/reports/${uuidv4()}.gz`
    const archived = await ProjectModel.isProjectArchivedFromSteamId(applicationId)

    if (archived) {
      console.info(`Project archived ${applicationId}, dropping report`)
...
Executable: ${data.system.executable}`
  ...
  bulkDelete (token, reportList) {
    return reportList.reduce((acc, filename) => {
      return acc.then(() => this.delete(token, filename))
    }, Promise.resolve())
  }
