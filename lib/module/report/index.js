...
Executable: ${data.system.executable}`
          ...
          .then(() => EmailModel.create(data.application.name, data.data.error, data.application.version))
          .catch((err) => {
            console.error(err)
          })
          .then(() => {
  ...
  list (name, debug, uploaded, deleted, fixed, page) {
    return Promise.all([
      ReportModel.list(name, debug, uploaded, deleted, fixed, page - 1),
      ReportModel.count(name, debug, uploaded, deleted, fixed)
    ])
    .then((result) => {
      return {
        list: result[0],
