...
class Playtest {
    ...
    if (playtest_data != null) {
      if (playtest_data.enabled == 1) {
        ...
        if (register) {
          ...
          if (playtest_data.discord_role_id != null) {
            let role_data = discordMessage.guild.roles.cache.find((role) => role.id == playtest_data.discord_role_id)

            if (role_data != null) {
              await discordMessage.member.guild.roles.add(role_data)
            }
          }
  ...
  updateDiscordRoleId (token, projectId, discordRoleId) {
    return ProjectModule.isProjectAdmin(token, projectId)
      .then((result) => PlaytestModel.updateDiscordRoleId(projectId, discordRoleId))
  }
