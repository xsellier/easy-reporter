...
class Project {
  create (token, steamId, name, secret, email, apiToken) {
    return Promise.resolve()
      .then(() => AuthenticationModule.getUserId(token))
      .then((userId) => ProjectModel.create(name, steamId, userId, secret, email, apiToken))
  }
  ...
  listMember (token, projectId) {
    return Promise.resolve()
      .then(() => AuthenticationModule.getUserId(token))
      .then((userId) => ProjectModel.isProjectMember(projectId, userId))
