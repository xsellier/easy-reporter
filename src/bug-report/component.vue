  ...
  <template v-slot:append>
    <v-footer>
      <v-row justify="center" no-gutters>
        <v-pagination v-model="currentPage" :length="totalPages" v-on:input="changePage()" v-on:next="emitUpdateSignal()" v-on:previous="emitUpdateSignal()" size="x-small" total-visible=3></v-pagination>
      </v-row>
    </v-footer>
  </template>
</v-navigation-drawer>
